#!/usr/bin/env bash
# Локальная сборка цели SwiftlyS2: restore -> тесты -> Release -> архив.
set -euo pipefail
...
ZIP="$(ls -1 bin/Release/net10.0/NotifyMessages_swiftly_*.zip 2>/dev/null | head -n 1)"
if [[ -z "$ZIP" || ! -f "$ZIP" ]]; then
  echo "Архив не собрался: bin/Release/net10.0/NotifyMessages_swiftly_*.zip" >&2
  exit 1
fi
