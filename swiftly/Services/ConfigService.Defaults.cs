    ...
    private static MessagesConfig CreateDefaultMessages()
    {
        return new MessagesConfig
        {
            LanguageMessages = new Dictionary<string, Dictionary<string, string>>
            {
                ...
                // Оповещение о рестарте (sw_restart_notify)
...
------------------------------------------------------------------------------
  sw_nm_check                 проверить шаблоны: неизвестные теги, дыры в переводах
  sw_nm_preview welcome       показать приветствие себе прямо сейчас
  sw_nm_preview ad 1          показать первый блок рекламы
  sw_nm_preview key prefix    показать один ключ из Messages.json
  sw_nm_preview raw {RED}тест произвольный текст с тегами
  sw_reload_advert            применить изменения всех четырёх файлов
...
Где какой работает — скажет sw_nm_check.

ЕСЛИ ЧТО-ТО НЕ РАБОТАЕТ
------------------------------------------------------------------------------
Тег виден игроку как текст в скобках   -> sw_nm_check покажет, где он лишний
