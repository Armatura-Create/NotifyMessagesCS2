...
- **Интерфейс `IKHook` плагин берёт из заголовков Metamod, против которых собран**, а реализацию
  даёт ядро Metamod на сервере. CI собирает против `metamod-source` master, поэтому при смене
  `khook.hpp` сверять не только порядок слотов, но и **сигнатуры**: в git1467 (`7e24ce9e`,
  2026-09-10) `RemoveHook` получил два параметра в том же слоте, и сборка против git1466 при
  выгрузке передавала ядру мусор вместо `hook_removal_fn` — так сломался ConnectHistory 3.1.3.
  Новые хвостовые параметры безопасны в обратную сторону (новая сборка на старом ядре), поэтому
  минимум остаётся git1460, но **такая правка в Metamod означает новый релиз**: лечит пересборка,
  а до выгрузки плагина ничего не падает.
...
Шаг идемпотентен — повторный запуск дозаливает файлы и обновляет описание. Ассеты грузятся
по одному, каждый под `timeout 240` с тремя попытками и `--clobber`: загрузка на GitHub иногда
зависает без ошибки (v2.3.2 простоял так 10 минут на втором архиве), а у задачи `publish`
стоит `timeout-minutes: 20`. Черновик создаётся без файлов и публикуется только когда все
ассеты на месте — полурелиза не бывает.
...
- **При смене карты `player_disconnect` не приходит, а `player_connect_full` приходит заново.**
  На этом стоит подавление спама: второй `connect_full` у игрока, который уже числится
  fully-connected в `SessionService`, — возврат на новую карту: без приветствия, без анонса
  входа, и его первое попадание в команду (`player_team` 0→N) тоже не анонсируется
  (`MarkReturning`/`TakeReturning`, срок `ReturningGrace` = 90 с — дольше в спектаторах
  значит настоящий выход в команду). После hot reload все текущие игроки помечаются
  fully-connected в `Load()`, иначе первая смена карты анонсировала бы их как новых.
- **Смена сторон (halftime, `mp_swapteams`) не анонсируется — два независимых фильтра.**
  Первый — поле `player_team.silent`: движок выставляет его на своих «тихих» сменах и сам
  ничего не пишет в чат. Второй — `TeamSwap.IsMassSwap`: переходы T↔CT копятся кадр
  (`Server.NextFrame` / `Scheduler.NextTick`) и пачка ≥ 2 в одном кадре отбрасывается —
  движок меняет стороны всем синхронно, один игрок так не переходит. Через кадр уходят
  только строки и числа, контроллер — нет.
- **Состояние игрока чистится в `EventPlayerDisconnect` целиком** — сессия, метка возврата, язык клиента,
